...
Starting a focus session writes the actual start time immediately. Closing the focus window minimizes it without ending the session. The mini timer hides while the full timer is open. After closing it, click the floating mini timer to reopen it or drag it to any position; its position is saved. On desktop, the status-bar item also restores it. The session is saved and can be restored after Obsidian restarts.
